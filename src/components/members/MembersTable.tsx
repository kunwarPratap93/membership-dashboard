...
import type { Member, SortOptions, SortField, MembershipType } from '../../types/member';
...
interface MembersTableProps {
  ...
  onUpdateType: (id: string, type: MembershipType) => void;
...
export const MembersTable: React.FC<MembersTableProps> = ({
  ...
  // Get tier style
  const getTierBadge = (tier: MembershipType) => {
    switch (tier) {
      case 'Enterprise':
              ...
              >
                ...
                <td className="py-3.5 px-6">
                  <select
                    ...
                    onChange={(e) => onUpdateType(member.id, e.target.value as MembershipType)}
        ...
        {members.map((member) => (
          ...
          >
            ...
            <div className="flex items-center justify-between text-xs pt-1">
              <span className="text-slate-450 dark:text-slate-500">Status Status</span>
              <button
                onClick={() => onToggleStatus(member.id)}
